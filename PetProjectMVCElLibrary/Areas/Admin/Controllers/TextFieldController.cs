            ...
            if (CheckUser.IsUserTry(_httpContextAccessor, out userId))
            {
				...
				if (await _applicationUserService.IsUserRoleConfirm(Guid.Parse(userId.ToString()), "admin"))
				{
					try
					{
						...
						TextFieldDTO? textFieldDTO = codeWord == default ? new TextFieldDTO() : (await _textFieldService.GetTextFieldByCodeWord(codeWord));
            ...
            if (ModelState.IsValid)
            {
                ...
                if (CheckUser.IsUserTry(_httpContextAccessor, out userId))
                {
                    try
                    {
                        ...
                        {
							TempData["Message"] = "Не удалось изменить текстовое поле!";
						}
					}
                    catch (Exception ex)
                    {
